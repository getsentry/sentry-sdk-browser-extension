...
export default function Home() {
  ...
  return (
    <section>
      {isLoading() ? Loading() : Loaded(sdkInfo(), options())}
    </section>
  );
}
...
function WithSdk(sdkInfo: SdkInfo, options: Options) {
  return (
    <>
      <div>SDK {sdkInfo.name} with version {sdkInfo.version} loaded.</div>
      <ul>
        {Object.entries(options).map(([key, value]) => (
          <li>{key}: {serializeOption(key as keyof Options, value)}</li>
        ))}
      </ul>
    </>
  )
...
function WithoutSdk() {
  return (
    <div>Sentry SDK not detected.</div>
  )
}
